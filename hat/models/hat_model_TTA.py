...
class HATModel(SRModel):
    ...
    def _forward_net_g(self, x):
        """Forward with net_g_ema if available, otherwise net_g."""
    ...
    def self_ensemble_process(self, x):
        """x8 self-ensemble test-time augmentation for SR."""
        def _transform(v, op):
            if op == 'v':
        ...
        # Generate 8 augmented inputs
        lq_list = [x]
        for tf in ['v', 'h', 't']:
            lq_list.extend([_transform(t, tf) for t in lq_list])

        # Forward each augmented input
        out_list = []
        for aug in lq_list:
            out = self._forward_net_g(aug)
            out_list.append(out)
        ...
        # Average outputs
        output = torch.stack(out_list, dim=0).mean(dim=0)
        return output
    
    def process(self):
        with torch.no_grad():
            if self.opt['val'].get('selfensemble', False):
                self.output = self.self_ensemble_process(self.img)
            else:
                self.output = self._forward_net_g(self.img)
            # self.net_g.train()
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                try:
                    with torch.no_grad():
                        if self.opt['val'].get('selfensemble', False):
                            output_tile = self.self_ensemble_process(input_tile)
                        else:
                            output_tile = self._forward_net_g(input_tile)
