...
def process_image(image_input_path, image_output_path, model, device):
    with torch.no_grad():
        ...
        aug_pairs = [
            ...
            (lambda x: torch.rot90(x, 1, [1, 2]), lambda x: torch.rot90(x, 3, [1, 2])),
            (lambda x: torch.rot90(x, 2, [1, 2]), lambda x: torch.rot90(x, 2, [1, 2])),
            (lambda x: torch.rot90(x, 3, [1, 2]), lambda x: torch.rot90(x, 1, [1, 2])),
